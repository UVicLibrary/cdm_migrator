		...
		def generate
			...
			records.each do |rec|
				if rec.last == "cpd"
					...
					url = "#{@cdm_url}/utils/getfile/collection/#{params[:collection]}/id/#{rec.first}/filename/#{rec.first}.#{rec.last}" unless params[:file_system]=="true" and @cdm_api=="front" #"file://#{file_path(rec.first)}"
					url = "#{@cdm_url}:#{@cdm_port}/cgi-bin/showfile.exe?CISOROOT=/#{params[:collection]}&CISOPTR=#{rec.first}" unless params[:file_system]=="true" and @cdm_api=="server"
